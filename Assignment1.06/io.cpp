    ...
    for (x = 0; x < MAP_X; x++) {
      if (world.cur_map->cmap[y][x]) {
        if(world.cur_map->cmap[y][x]->symbol == '@'){
          if(y == (uint32_t)world.pc.pos[dim_y] && x == (uint32_t)world.pc.pos[dim_x]){
  ...
  if (world.cur_map->map[dest[dim_y]][dest[dim_x]] == ter_gate) {
    ...
    if(dest[dim_y] == 0){
      ...
      if (world.cur_idx[dim_y]) {
        world.cur_idx[dim_y]--;
        new_map(world.cur_idx[dim_y]);
      }
      world.pc.pos[dim_y] = 20;
      ...
      if (world.cur_idx[dim_x] < WORLD_SIZE - 1) {
        world.cur_idx[dim_x]++;
        new_map(world.cur_idx[dim_x]);
      }
      world.pc.pos[dim_y] = dest[dim_y];
      world.pc.pos[dim_x] = 0;
    }
...
void io_handle_input(pair_t dest)
{
  ...
  do {
    ...
    case 'f':
      int x, y;
      mvprintw(0, 0, "Enter the x and y for the map you seek: ");
      //turn  echo() and cursor()
      echo();
      curs_set(1);
      mvscanw(0, 40,"%d %d", &x, &y);
      noecho();
      curs_set(0);
      //refresh();
      //int p;
