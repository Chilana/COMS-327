...
void io_display()
{
 ...
 // int pokeID;
  ...
  for (y = 0; y < MAP_Y; y++) {
    for (x = 0; x < MAP_X; x++) {
      if (world.cur_map->cmap[y][x] && world.cur_map->cmap[y][x]->symbol != 'P') {
  ...
  * not otherwise used.                                                 */
  ...
  /*for(auto poke: world.cur_map->pokeVec){
      //mvprintw(0, 0, "PENIS");
      attron(COLOR_PAIR(COLOR_RED));
      mvaddch(poke->y, poke->x, toupper(poke->name[0]));
      attroff(COLOR_PAIR(COLOR_RED));
...
void io_pick_poke(){
  mvprintw(0, 0, "Pick one of these three pokemon");
  mvprintw(1, 0, "Option 1: bulbasaur");
  mvprintw(2, 0, "Option 2: charmander");
  mvprintw(3, 0, "Option 3: squirtle");
  refresh();
  int ch = getch();
  switch(ch){
    case 1:
      strcpy(world.pc.poke.name, "bulbsaur");
      world.pc.poke.id = 3;
      
      break;
    case 2:
      strcpy(world.pc.poke.name, "charmander");
      world.pc.poke.id = 4;
      break;
    case 3:
      strcpy(world.pc.poke.name, "squirtle");
      world.pc.poke.id = 7;
      break;
    default:
      mvprintw(4, 0, "Enter a vaild option");
      break;
  }
  world.pc.poke.level = 1;
}
...
void io_battle(character *aggressor, character *defender)
{
  ...
  io_display();
  int i = 0;
  for(Pokemon poke : n->poke){
    if(poke.level > 0){
      mvprintw(i, 0, "Hey! My pokemon is %s and is level %d, hp: %d attack: %d defense: %d  special-attack: %d specical-defense: %d speed: %d. The moves I know are: %s , %s" , pokemon[poke.id].identifier, poke.level, poke.calcHP(pokemon_stats[poke.id * 6 - 5].base_stat, poke.iv, poke.level),
        poke.calc_other_stat(pokemon_stats[poke.id * 6 - 4].base_stat, poke.iv, poke.level), poke.calc_other_stat(pokemon_stats[poke.id * 6 - 3].base_stat, poke.iv, poke.level), 
        poke.calc_other_stat(pokemon_stats[poke.id * 6 - 2].base_stat, poke.iv, poke.level), poke.calc_other_stat(pokemon_stats[poke.id * 6 - 1].base_stat, poke.iv, poke.level)
        , poke.calc_other_stat(pokemon_stats[poke.id * 6].base_stat, poke.iv, poke.level), poke.moves[0], poke.moves[1]); // pokeid * 6 - 5 -- for HP;
      i++;
    }
  }
  refresh();
...
void io_pokebattle(pair_t place){
  Pokemon *p = new Pokemon();
  int j;
  int pokeId = rand() % 1092;
  p->id = pokeId;
  p->level = level();
  ...
  for(int i = 0; i < 2; i++){
    do{
       j = rand() % 528239;
    } while(pokemon_moves[j].level > p->level);
    strcpy(p->moves[i], moves[pokemon_moves[j].move_id].identifier);
  }

  io_display();
  mvprintw(0, 0, "I'm such a wild pokemon and my name is %s my level %d, hp:  %d attack: %d defense: %d  special-attack: %d specical-defense: %d speed: %d. The moves I know are: %s , %s, shiny : %d",
   pokemon[p->id].identifier, p->level, p->calcHP(pokemon_stats[p->id * 6 - 5].base_stat, p->iv, p->level), p->calc_other_stat(pokemon_stats[p->id * 6 - 4].base_stat, p->iv, p->level)
   , p->calc_other_stat(pokemon_stats[p->id * 6 - 3].base_stat, p->iv, p->level), p->calc_other_stat(pokemon_stats[p->id * 6 - 2].base_stat, p->iv, p->level) , p->calc_other_stat(pokemon_stats[p->id * 6 - 1].base_stat, p->iv, p->level)
   , p->calc_other_stat(pokemon_stats[p->id * 6].base_stat, p->iv, p->level), p->moves[0], p->moves[1], p->shiny);
  mvprintw(place[dim_y], place[dim_x], "P");
  refresh();
  getch();
}
