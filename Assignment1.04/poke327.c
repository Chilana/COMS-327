...
/*static char terType(int x, int y){
  switch(world.cur_map->map[x][y]){
    case: 
  }
}*/
...
static void print_map()
{
  ...
  for (y = 0; y < MAP_Y; y++)
  {
    for (x = 0; x < MAP_X; x++)
    {
      ...
      else if (world.cur_map->charT[y][x] != NULL)
      { 
      //  printf("|");
        switch (world.cur_map->charT[y][x]->c)
        {
        case char_rival:
          putchar(RIVAL);
          break;
...
/*void place_wanderer(){
  ...
  do {
    x = rand() % (MAP_X - 2) + 1;
    y = rand() % (MAP_Y - 2) + 1;
  } while (world.cur_map->map[y][x] != ter_water && world.cur_map->map[y][x] != ter_path &&);

  world.npc.wNPC.pos[dim_y] = y;
  world.npc.wNPC.pos[dim_x] = x;


  
}*/

static int32_t char_cmp(const void *c1, const void *c2) {
    int32_t cmp = ((char_t *)c1)->nextTurn - ((char_t *)c2)->nextTurn;

    if(cmp != 0){
      return cmp;
    }
    else{
      return ((char_t *)c1)->seqNum - ((char_t *)c2)->seqNum;
    }
}
...
void init_pc()
{
  ...
  char_t *p;
  p = malloc(sizeof(char_t));
  p->seqNum = 0;
  p->c = char_pc;
  
  world.cur_map->charT[y][x] = p;
  //free(t);
  heap_insert(&npcH, &world.pc);
}

void init_npc(int numT){
  int x, y;
  char_t *cT;

  for (y = 1; y < MAP_Y - 1; y++) {
    for (x = 1; x < MAP_X - 1; x++) {
      world.cur_map->charT[y][x] = NULL;
    }
  }

  //world.cur_map->charT[y][x] = p;
  //world.charT[y][x] = &world.pc.pos[dim_y][dim_x];
  for(int i = 0; i < numT; i++){
    //printf("%d", i);
    cT = malloc(sizeof(char_t));
    ...
    do
    {
      cT->pos[dim_y] = rand() % (MAP_Y - 2) + 1;
      cT->pos[dim_x] = rand() % (MAP_X - 2) + 1;
    } while (world.cur_map->charT[y][x] != NULL ||
             move_cost[cT->c][world.cur_map->map[cT->pos[dim_y]][cT->pos[dim_x]]] == DIJKSTRA_PATH_MAX || world.cur_map->map[cT->pos[dim_y]][cT->pos[dim_x]] == ter_path
              || world.cur_map->map[cT->pos[dim_y]][cT->pos[dim_x]] == ter_water);

      world.cur_map->charT[cT->pos[dim_y]][cT->pos[dim_x]] = cT;
      cT->ter = world.cur_map->map[y][x];
...
void move_npc(){
    ...
    while((t = heap_remove_min(&npcH)) && t->c != char_pc){


        switch(t->c){
          case char_hiker:
            
            cost = move_cost[char_hiker][world.hiker_dist[cur[dim_y]][cur[dim_x] + 1]];

            world.cur_map->charT[t->pos[dim_y]][t->pos[dim_x]] = t;
            cur[dim_y] = t->pos[dim_y];
            cur[dim_x] = t->pos[dim_x];

            world.cur_map->charT[cur[dim_y]][cur[dim_x]] = NULL;
            if (move_cost[char_hiker][world.hiker_dist[cur[dim_y]][cur[dim_x]++]] < cost){
                move[dim_y] = cur[dim_y];
                move[dim_x] = cur[dim_x]++;
            }
            else if (move_cost[char_hiker][world.hiker_dist[cur[dim_y]--][cur[dim_x]++]] < cost){
                move[dim_y] = cur[dim_y]--;
                move[dim_x] = cur[dim_x]++;
            }
            else if (move_cost[char_hiker][world.hiker_dist[cur[dim_y]++][cur[dim_x]++]] < cost){
            ...
            t->pos[dim_y] = move[dim_y];
            t->pos[dim_x] = move[dim_x];
          ...
          case char_rival:
            ...
            cost = move_cost[char_rival][world.rival_dist[cur[dim_y]][cur[dim_x] + 1]];
            ...
            if (move_cost[char_rival][world.rival_dist[cur[dim_y]][cur[dim_x]++]] < cost){
                move[dim_y] = cur[dim_y];
                move[dim_x] = cur[dim_x]++;
            }
            else if (move_cost[char_rival][world.rival_dist[cur[dim_y]--][cur[dim_x]++]] < cost){
                move[dim_y] = cur[dim_y]--;
                move[dim_x] = cur[dim_x]++;
            }
            else if (move_cost[char_rival][world.rival_dist[cur[dim_y]++][cur[dim_x]++]] < cost){
          ...
          case char_pacer:
            ...
            world.cur_map->charT[cur[dim_y]][cur[dim_x]] = NULL;
            if(dir == 'e'){
              if(world.cur_map->map[cur[dim_y]][cur[dim_x]++] == t->ter){
                cur[dim_x]++;
              }
              /*else{
                t->dir == 'w';
              ...
              if(world.cur_map->map[t->pos[dim_y]][t->pos[dim_x]--] == t->ter){
                cur[dim_x]--;
              }
              /*else{
                t->dir == 'e';
            ...
            t->pos[dim_y] = cur[dim_y];
            t->pos[dim_x] = cur[dim_x];
          ...
          case char_wanderer:
            ...
            ter = t->ter; 
            world.cur_map->charT[cur[dim_y]][cur[dim_x]] = NULL;
            if(world.cur_map->map[cur[dim_y]][cur[dim_x]++] == t->ter && dir == 'e'){
              cur[dim_x]++;
            }
            else if(world.cur_map->map[cur[dim_y]][cur[dim_x]--] == t->ter && dir == 'w'){
              cur[dim_x]--;
              
            }
            else if(world.cur_map->map[cur[dim_y]++][cur[dim_x]] == t->ter && dir =='n'){
              cur[dim_y]++;
            }
            else if(world.cur_map->map[cur[dim_y]--][cur[dim_x]] == ter && dir == 's'){
              cur[dim_y]--;
            }
            t->pos[dim_y] = cur[dim_y];
            t->pos[dim_x] = cur[dim_x];
          ...
          case char_explorer:
            ...
            dir = t->dir;
            world.cur_map->charT[cur[dim_y]][cur[dim_x]] = NULL;
            if(world.cur_map->map[cur[dim_y]][cur[dim_x]++] != ter_gate){
              cur[dim_x]++;
            }
            else if(world.cur_map->map[cur[dim_y]][cur[dim_x]--] != ter_gate){
              cur[dim_x]--;
        ...
        t->nextTurn += move_cost[t->c][world.cur_map->map[t->pos[dim_y]][t->pos[dim_x]]];
        heap_insert(&npcH, t);
    }
}
...
int main(int argc, char *argv[])
{
  ...
  printf("How many trainers do you want? (Enter zero to let the computer decide): ");
  scanf("%d", &numTrainers); 
  if(numTrainers == 0){
    numTrainers = 10;
  }
  heap_init(&npcH, char_cmp, NULL);
  init_world();
  init_pc();
